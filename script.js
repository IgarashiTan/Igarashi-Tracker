...
function initDateTimePicker() {
    flatpickr("#datetimeInput", {
        ...
        dateFormat: "Y-m-d\\TH:i", // Formato compatível com o que já usamos (YYYY-MM-DDThh:mm)
        time_24hr: true, // Formato 24 horas
        locale: "pt", // Localização para português
        allowInput: true, // Permite que o usuário digite manualmente
        altInput: true, // Exibe um formato mais amigável para o usuário
        altFormat: "d/m/Y H:i", // Formato de exibição (ex.: 05/05/2025 14:30)
        minuteIncrement: 1 // Define o incremento dos minutos para 1
    });
}
...
function setToday() {
    ...
    document.getElementById('datetimeInput').value = todayFormatted;

    // Também atualizamos o Flatpickr para refletir o valor
    const flatpickrInstance = document.getElementById('datetimeInput')._flatpickr;
    flatpickrInstance.setDate(todayFormatted);
}
...
function resetToRealTime() {
    ...
    datetimeInput.value = '';
    const flatpickrInstance = datetimeInput._flatpickr;
    flatpickrInstance.clear(); // Limpa o Flatpickr
    if (interval) {
...
function simulatePass(encodedTLE, encodedSatName, passTimeLocal, encodedLat, encodedLng, encodedTimezone) {
    ...
    const datetimeInput = document.getElementById('datetimeInput');
    datetimeInput.value = passTimeFormatted;
    const flatpickrInstance = datetimeInput._flatpickr;
    flatpickrInstance.setDate(passTimeFormatted);
...
function startTracking() {
    ...
    // Calcular e exibir a inclinação orbital (em graus)
    const inclinationDegrees = satellite.degreesLat(satrec.inclo); // Converte de radianos para graus
    document.getElementById('inclination').textContent = inclinationDegrees.toFixed(2);
...
window.onload = function() {
    ...
    initDateTimePicker(); // Inicializa o Flatpickr
    loadFromUrlParams();
};